...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None,if_t=False):
        ...
        if labels is not None and mask is not None:
            ...
            mask = torch.eye(batch_size, dtype=torch.float32).to(device)    #生成对角线为1，其余部分为0的二维数组
        ...
        anchor_dot_contrast = torch.div(    #32 * 32 
            ...
            self.temperature)
        # for numerical stability   数值稳定
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()   #32*32 对角线为0，其余为负数

        # tile mask
        mask = mask.repeat(anchor_count, contrast_count)   #32*32
        # mask-out self-contrast cases
        logits_mask = torch.scatter(      #对角线为0，其余为1
            torch.ones_like(mask),
            1,
            torch.arange(batch_size * anchor_count).view(-1, 1).to(device),
            0
        )
        mask = mask * logits_mask   # 大对角线为0，小对角线为1，其余为0，代表配对真实标签
        '''
        0   1
          0   1
        1   0
        ...
        # tempture = 2
        # exp =  torch.exp(logits/tempture) * logits_mask
        # # exp_logits = torch.exp(logits)   
        # similar_matrix = exp/exp.sum(1,keepdim=True)
        # if if_t: return similar_matrix
        # else:
        #     similar_matrix = (similar_matrix+torch.eye(batch_size*contrast_count).to(device)).log()


        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))
        
        #kl = F.kl_div(similar_matrix.log(), similar_matrix.T.softmax(dim=-1), reduction='sum')
...
class MoCo(nn.Module):
    ...
    """
    def __init__(self,backbone, pretrained=True,head='mlp',dim_in=2048,feat_dim=128, K=7200, m=0.999, T=0.07):
        """
    ...
    def forward(self, im_q, im_k,im_healthy,is_train=True,is_student=True,is_eliminate_falseN=True):
        """
        Input:
        ...
        if is_eliminate_falseN:
            ...
            with torch.no_grad():  # no gradient to keys
                if is_train and is_student:    #如果训练则更新encoder_k，否则不更新
            ...
            # positive logits: Nx1
            l_pos = torch.einsum('nc,nc->n', [q, k]).unsqueeze(-1)    #einstan 求和约定，用于简洁的表示乘积、点积、转置
            # negative logits: NxK
