...
def student_train(t_model,model, train_config, data_config, train_dataset, val_dataset, save_path, device,cl_lamda,kd_lamda,checkpoint=None, logger=None):
    ...
    for epoch in range(start_epoch, train_config['epochs'] + 1):
        ...
        for step, train_data in progress:
            ...
            output,target = model(X_1,X_2)   #batchsize*(K+1) , batchsize
            loss1 = loss_function(output,target)

            tempture = 4
            with torch.no_grad():
                output_t,_ = t_model(X_1,X_2,is_train=True,is_student=False)  #为了不更新教师模型的负样本的encoder

            loss2 = F.kl_div(torch.softmax(output/tempture,dim=1).log(),torch.softmax(output_t/tempture,dim=1),reduction='batchmean')*tempture*tempture
            loss = cl_lamda*loss1 + kd_lamda* loss2 
            ...
            kd_loss += loss2.item()
            avg_kd_loss = kd_loss / (step + 1)
            cl_loss += loss1.item()
            avg_cl_loss = cl_loss / (step + 1)
        ...
        if epoch % 10 == 0:
            ...
            if indicator < min_indicator:
                torch.save(model.state_dict(), os.path.join(save_path, 'best_validation_model.pt'))
        ...
        if logger:
            ...
            logger.add_scalar('kd_loss', avg_kd_loss, epoch)
            logger.add_scalar('cl_loss', avg_cl_loss, epoch)
...
def eval(model, dataloader, loss_function, device):
    ...
    for step, test_data in enumerate(dataloader):
        ...
        output,target = model(X_1,X_2,is_train=False)
